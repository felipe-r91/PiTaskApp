...
export function Broadcast() {
  ...
  const [cardsWeek1, setCardsWeek1] = useState<Card[]>([])
  const [cardsWeek2, setCardsWeek2] = useState<Card[]>([])
  const [cardsWeek3, setCardsWeek3] = useState<Card[]>([])
  ...
  const refCardsWeek1 = useRef<Card[]>([])
  const refCardsWeek2 = useRef<Card[]>([])
  const refCardsWeek3 = useRef<Card[]>([])
  const emptyAgenda1 = useRef<boolean>()
  const emptyAgenda2 = useRef<boolean>()
  const emptyAgenda3 = useRef<boolean>()
  ...
  useEffect(() => {
    ...
    if (weekToShow === 0) {
      ...
      if (update) {
        setCardsWeek1(createCards(eventsToUse))
        refCardsWeek1.current = cardsWeek1
        console.log(refCardsWeek1.current)
      }
      if (refCardsWeek1.current.length === 0) {
        emptyAgenda1.current = true
      } else {
        emptyAgenda1.current = false
      }
    }
    if (weekToShow === 1) {
      ...
      const eventsToUse = getEarliestAndLatestEvents(eventsOnThisWeek)
      if (update) {
        setCardsWeek2(createCards(eventsToUse))
        refCardsWeek2.current = cardsWeek2
      }
      if (refCardsWeek2.current.length === 0) {
        emptyAgenda2.current = true
      } else {
        emptyAgenda2.current = false
      }
    }
    if (weekToShow === 2) {
      ...
      const eventsToUse = getEarliestAndLatestEvents(eventsOnThisWeek)
      if (update) {
        setCardsWeek3(createCards(eventsToUse))
        refCardsWeek3.current = cardsWeek3
      }
      if (refCardsWeek3.current.length === 0) {
        emptyAgenda3.current = true
      } else {
        emptyAgenda3.current = false
      }
    }
    setShow(false)
              ...
              {emptyAgenda1.current ?
                ...
                <>
                  <div className="px-10 pt-7 grid grid-rows-3 grid-flow-col gap-2.5">
                    ...
                    {refCardsWeek1.current.map((card, i) => {
                      return (
                        <div key={i}>
                          <Transition
                            appear={true}
              ...
              {emptyAgenda2.current ?
                ...
                <>
                  <div className="px-10 pt-7 grid grid-rows-3 grid-flow-col gap-2.5">
                    ...
                    {refCardsWeek2.current.map((card, i) => {
                      return (
                        <div key={i}>
                          <Transition
                            appear={true}
              ...
              {emptyAgenda3.current ?
                ...
                <>
                  <div className="px-10 pt-7 grid grid-rows-3 grid-flow-col gap-2.5">
                    ...
                    {refCardsWeek3.current.map((card, i) => {
                      return (
                        <div key={i}>
                          <Transition
                            appear={true}
